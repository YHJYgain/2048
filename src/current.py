...
def __init__(self,scores_file="../assets/scoreRecord.db",
             script_file="../initScoreRecord.sql", colors=None,
    ...
    utils.initialize_database(script_file, scores_file)
    self.score = 0
...
def loadBestScore(self):
    ...
    """
    try:
        self.best_score = utils.get_highest_score_record(self.scores_file)
    except:
        return False
    return True
...
def saveScore(self):
    """
        ...
        """
    try:
        utils.add_record(self.scores_file, self.scores, datetime.datetime.now())
    except:
        return False
    return True
